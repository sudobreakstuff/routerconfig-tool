...
class WinboxTerminalClient:
    ...
    def _strip_ansi(text: str) -> str:
        import re as _re
        if WinboxTerminalClient.ANSI_RE is None:
            WinboxTerminalClient.ANSI_RE = _re.compile(r'\x1b\[[0-9;?]*[A-Za-z]|\x1b\][^\x07]*(\x07|\x1b\\)|[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]')
        text = WinboxTerminalClient.ANSI_RE.sub('', text)
        return text

    def exec_command(self, command: str, timeout: float = 10.0) -> str:
        """Run a single RouterOS CLI command and return cleaned text output.

        Sends the command to the terminal, collects output until the prompt
        returns, then strips ANSI codes, the echoed command line, and the
        trailing prompt so callers get plain `key: value` text just like SSH.
        """
        self.send_ready_signal()
        time.sleep(0.3)
        # Drain the banner/prompt fully
        self._drain_until_prompt(timeout=2.0)

        self.send_terminal_input(command.encode('utf-8') + b'\r')

        deadline = time.time() + timeout
        buf = b''
        while time.time() < deadline:
            out = self.receive_terminal_output(timeout=0.3)
            if out:
                buf += out
            text = self._strip_ansi(buf.decode('utf-8', errors='replace'))
            if self._has_prompt_after_command(text, command):
                break

        text = self._strip_ansi(buf.decode('utf-8', errors='replace'))
        text = text.replace('\r', '\n')
        # Drop echoed command lines (the terminal redraws the command, so it
        # can appear more than once) — keep everything after the last echo.
        if command in text:
            parts = text.split(command)
            text = parts[-1]
        # Drop trailing prompt fragment(s)
        text = text.rsplit('] >', 1)[0] if '] >' in text else text
        # Some redraws leave a bare prompt like `[admin@host` at the end
        import re as _re
        m = _re.search(r'\[[^\]]*@[^\]]*$', text)
        if m:
            text = text[:m.start()]
        # Drop stray helper/control lines
        lines = []
        for ln in text.split('\n'):
            if '9999B' in ln or '[?]' in ln or ln.strip() == '..' or 'Gives the list of available' in ln or 'Completes the command' in ln or 'Move up' in ln or 'Use command at' in ln or 'Press Ctrl' in ln:
                continue
            lines.append(ln)
        text = '\n'.join(lines)
        return text.strip()
    ...
    @staticmethod
    def _has_prompt_after_command(text: str, command: str) -> bool:
        """True once a prompt (`] >`) appears after the echoed command line."""
        if command not in text:
            return False
        pos = text.rfind(command)
        tail = text[pos + len(command):]
        return '] >' in tail

    def _drain_until_prompt(self, timeout: float = 2.0):
        """Read and discard terminal output until the prompt has settled."""
        deadline = time.time() + timeout
        buf = b''
        while time.time() < deadline:
            out = self.receive_terminal_output(timeout=0.3)
